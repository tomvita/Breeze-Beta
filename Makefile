...
TARGET		:=	Breeze
BUILD		:=	build
SOURCES		:=	source nanovg/shaders minizip/source2
DATA		:=	data
INCLUDES	:=	include ../include minizip/include libs/nlohmann
ROMFS		:=	romfs

# Output folders for autogenerated files in romfs
OUT_SHADERS	:=	shaders

APP_TITLE     := a game cheating tool
APP_AUTHOR    := Tomvita
APP_VERSION   := Beta 11
#0.0.6
...
CXXFLAGS	:= $(CFLAGS) -std=gnu++17 -fno-exceptions -fno-rtti -fexceptions  

ASFLAGS	:=	-g $(ARCH)
LDFLAGS	=	-specs=$(DEVKITPRO)/libnx/switch.specs -g $(ARCH) -Wl,-Map,$(notdir $*.map)

LIBS	:= -lnanovg -ldeko3d -lnx `curl-config --libs`
#-lcurl `freetype-config --libs`
