...
### Theory: How `make match` Works

Instead of relying on a single, unique instruction, this advanced technique creates a cheat that only activates when a specific **call chain** occurs. The `make match 1` and `make match all` buttons automate this by analyzing the execution flow that leads to your target instruction.

Every time a function is called, the processor needs to know where to return. This "return address" is the key. On AArch64, the return address is stored in the `X30` register or on the stack. By capturing the `X30` register and a few values from the top of the stack, Breeze can build a unique "fingerprint" for a specific code path.

For example, a generic `update_value` function might be called by hundreds of different parts of the game. However, the call chain for `Player -> update_stats -> update_value` is different from `Enemy -> update_stats -> update_value`. The `make match` tools generate a script that checks this fingerprint, ensuring your cheat code only runs when the player's value is being updated.

> For a deeper technical explanation of how return addresses and the call stack work, please see the [**X30 Match documentation**](x30%20match.md).
...
The `make match 1` and `make match all` buttons will then generate a complete script that checks these conditions before running your code.
...
3.  **Analyze and Generate**: In the **Gen2 Extra Menu**, analyze the captured data to find a reliable pattern. Once you select a promising data line, press `make match 1` or `make match all` to automatically generate the complete ASM cheat script.
    *   It is best to start with **`make match all`**. If it successfully isolates the target, you can then check if the less restrictive **`make match 1`** is sufficient (Use `Next stack_check` value to choose which value **`make match 1`** should use for matching).
